...
public class SecurityConfig {
    ...
    // HTTP 500(internal server error, 내부 서버 오류)를 발생함.
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests((authRequest) -> 
                ...
                .requestMatchers("/post/create", "/post/details", "/post/modify",
                        "/post/delete", "/post/update", "/api/reply/**")
                .hasRole("USER") // 위에서 설정한 페이지들이 USER 권한을 요구함을 설정.
//                .authenticated() // 권한여부에 상관업이 id, password가 일치하면 접근 가능
                .anyRequest() // .requestMatchers("/**"). 위 페이지들 이외의 모든 페이지
                .permitAll() // 권한없이 접근 허용.
                );
                */
        // 단점: 새로운 요청 경로, 컨트롤러를 작성할 때마다 Config 자바 코드를 수정해야함.
