...
public class ReplyService {
    ...
    public void update(long replyId, ReplyUpdateDto dto) {
        log.info("update(replyId={}, dto={})", replyId, dto);
        
        // 1. 댓글 아이디로 DB에서 엔터티를 검색(SELECT)
        Reply entity = replyRepository.findById(replyId).orElseThrow();
...
//        replyRepository.save(entity);
    ...
    public Reply create(ReplyCreateDto dto) {
        ...
        // 2. ReplyCreateDto 객체를 Reply 엔티티 객체로 변환.
        Reply entity = Reply.builder()
                .post(post)
                .replyText(dto.getReplyText())
                .writer(dto.getWriter())
